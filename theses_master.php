   <?php
    require_once("include/utils.php");
      ...
      if (!$current_page){
       ...
       $checkedFacetsObj  = initObjWithVars('etablissement',  $checkedFacetsObj);
	   ...
	   $checkedFacetsObj->langueThese->$i = 'fr';
     ...
     $url = "http://www.theses.fr/?q=".urlencode($q)."&checkedfacets=".$checkedFacets."&maxnumber=".$limit."&sort=".$sort."&fq=dateSoutenance:".urlencode($dateSoutenance)."&zone1=motCleRAs&val1=Traduction&op1=OR&zone2=motCleRAs&val2=Traductologie&op2=OR&zone3=motCleRAs&val3=Linguistique&op3=OR";
	 // 
	 $stLabel = "";
      if ($statut == "encours")
      {
 ...
 <div id="facet-panel-collapse" class="panel-group collapse" style="height: 0px;">
	 ...
	 <div class="panel-collapse facet-content collapse in" id="facet-status" style="height: auto;">
		 <div class="panel-body facet-panel">
          ...
          <?php 
            echo 	getfacets($thesefr,'etablissement', $checkedFacetsObj,$_GET);
...
<div id="appliedParams" class="clearfix constraints-container">
 ...
 <?php
	...
	$currentUrlNoPage = preg_replace("/((&|\?)p_page=\d*)/", "", $currentUrl);
	...
	if(!empty($numFound) && $numFound != '0'){
                    ...
                    else { $prev = "<a href='$currentUrlNoPage&p_page=".($current_page - 1)."'> &laquo; Précédent</a>  |";}
                    
                    
                    if ($current_page == $nb_pages) {  $next = "";}
                    else { $next = "  | <a href='$currentUrlNoPage&p_page=".($current_page + 1)."'>Suivant &raquo;</a>";}
?>
